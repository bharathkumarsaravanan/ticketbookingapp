    ...
    function showSeats(){
        ...
        oldGuys.map((peps, index)=> {
            if(index < rCount){
                ...
                axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+peps.id,{seat:tempValue})
                ...
                if(index < rCount + rCount){
                    ...
                    axios.post('hhttps://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+peps.id,{seat:tempValue})
                    .then(response => setPassengers(response.data.return))
                    setSelected(prev => [...prev, tempValue])
                }
                else{
                    if(index < rCount + rCount + rCount){
                        ...
                        if(index < (rCount + rCount + rCount + rCount)){
                            ...
                            axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+peps.id,{seat:'waiting'})
                            .then(response => setPassengers(response.data.return))
                        }
                    }
                }
        ...
        females.map((female, index) => {
            if(index < rCount){
                ...
                axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+female.id,{seat:tempValue})
                .then(response => setPassengers(response.data.return))
                setSelected(prev => [...prev, tempValue])
            }
            else{
                if(index < rCount+rCount){
                    axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+female.id,{seat:"waiting.."})
                    .then(response => setPassengers(response.data.return))
                }else{
        ...
        males.map((male, index) => {
            if(index < rCount){
                let tempValue = (index * 6) + 3;
                axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+male.id,{seat:tempValue})
                .then(response => setPassengers(response.data.return))
                setSelected(prev => [...prev, tempValue])
            }else{
                axios.post('https://node-server-lilac.vercel.app/home/agent/'+user+'/booking/'+male.id,{seat:'waiting...'})
                .then(response => setPassengers(response.data.return)) 
            }
        })
    }
